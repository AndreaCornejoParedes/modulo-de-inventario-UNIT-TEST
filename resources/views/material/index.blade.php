...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{url('/material/create')}}" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i
                class="fas fa-plus-circle text-white-50"></i> Crear Medicamentos </a>
    </div>
                ...
                <table class="table table-bordered" style="text-align:center;"  id="dataTableMaterial" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$dataM->Stock}}</td>
                            <td><button value="{{$dataM->ID}}" id="showMaterial" 
                                onclick='showMaterial(this)' class="btn btn-info">
                                    <i class="fas fa-eye"></i>
                                </button></td>
                                <td>                                                        
                                <a class="btn btn-success" href="{{url('/material/'.$dataM->ID)}}">
                                    <i class="fas fa-edit"></i>
                                 </a>
                                 <button class="btn btn-danger btn-flat btn-md remove-material" data-id="{{ $dataM->ID }}" data-action="{{ route('Material.delete',$dataM->ID) }}" onclick="deleteConfirmation('{{$dataM->ID}}')">
...
<script>
    function deleteConfirmation(numparte){
        Swal.fire({
            title: "¿Estás Seguro?",
            html: 'El Medicamentos: <strong>'+numparte+'</strong> será eliminado permanentemente',
            icon: 'warning',
                ...
                $.ajax({
                    ...
                    success: function (results) {
                        if (results.status == 1) {
                            ...
                            Toast.fire({
                                icon: 'success',
                                html: 'El Medicamento <strong>'+numparte+'</strong> fue eliminado correctamente',
                            });
                            setTimeout(function () {
    ...
    function showMaterial(element){
        axios.get("showMaterial/"+element.value).then((response)=>{
            if(response.status===200 && response.data.length>0){
                Swal.fire({
                    icon: 'info',
                    title: 'Detalles del Medicamentos:',
                    html:
