...
function checkEntries(){
    ...
    let checkEntries = entries.filter(x => x.ID === "");
    if(entries.length === 0 ){
...
function searchProduct(element){
    if(element.value != ""){
        let entries = getEntries().filter(x => x.ID != "");
        let isRepeated = [...new Set(entries.map(x => x.ID))];
...
function getEntries(){
    ...
    for (let i = 1; i < numberNodes ; i++) {
        let row = {
            ID:tableEntries.childNodes[3].childNodes[i].childNodes[0].childNodes[0].value,
