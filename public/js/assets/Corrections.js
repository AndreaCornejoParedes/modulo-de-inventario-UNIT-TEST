...
function checkCorrections(){
    ...
    let checkCorrections = corrections.filter(x => x.ID === "");
...
function searchProduct(element){
    if(element.value != ""){
        let corrections = getCorrections().filter(x => x.ID != "");
        let isRepeated = [...new Set(corrections.map(x => x.ID))];
...
function getCorrections(){
    ...
    for (let i = 1; i < numberNodes ; i++) {
        let row = {
            ID:tableCorrections.childNodes[3].childNodes[i].childNodes[0].childNodes[0].value,
