...
class CorrectionRequestController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try{            
            ...
            foreach ($request->Correcciones as $correccion) {
                DB::select("call sp_insertar_correcciones(?,?,?)", array($conc,$correccion['ID'],$correccion['Diferencia'],));
            }
            $datos = DB::select("call sp_mostrar_solicitudes_habilitadas()");        
            return view('CorrectionRequest.index',compact('datos'));
        }catch (\Exception $e){
            $datos = DB::select("call sp_mostrar_solicitudes_habilitadas()");    
    ...
    public function show($id)
    {
        ...
        $correcciones = DB::select("call sp_correcciones_de_solicitud('".$id."')");
